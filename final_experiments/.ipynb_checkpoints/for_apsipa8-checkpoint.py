...
def info_trans_scoring_1(classes, orig_A, lim_A):
    s_cls = classes
...
print("q = 0", file = codecs.open("for_apsipa6.txt", 'a', 'utf-8'))

for i in range(2, 10):
    temp = 0
    combi_ni = fact_10//(factorial(i)*factorial(10 - i))
    for scls in itertools.combinations(classes, i):
        temp += info_trans_scoring_1(scls, orig_A1, lim_A1)
    print(f"{i}-classes: {temp/combi_ni}", sep = "\n", file = codecs.open("for_apsipa6.txt", 'a', 'utf-8'))
